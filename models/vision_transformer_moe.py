...
from .custom_moe_layer import FMoETransformerMLP
from models.gate_funs.noisy_gate import NoisyGate
from models.gate_funs.noisy_gate_vmoe import NoisyGate_VMoE

from utils.moe_utils import save_checkpoint
from .vision_transformer import matmul
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, moe=False, moe_mlp_ratio=-1, moe_experts=64,
                 moe_top_k=2, moe_gate_dim=-1, world_size=1, gate_return_decoupled_activation=False, gate_return_gated_activation=False,
                 moe_gate_type="noisy", vmoe_noisy_std=1, moe_mlp=None):
        super().__init__()
        self.moe = moe
        ...
        if moe:
            ...
            if moe_mlp is None:
                self.mlp = FMoETransformerMLP(num_expert=moe_experts, d_model=dim, d_gate=moe_gate_dim, d_hidden=moe_hidden_dim,
                                              ...
                                              gate_return_gated_activation=gate_return_gated_activation,
                                              vmoe_noisy_std=vmoe_noisy_std)
            else:
                # print("share the previous layer")
                self.mlp = moe_mlp
            self.mlp_drop = nn.Dropout(drop)
        else:
            self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, x, gate_inp=None):
        x = x + self.drop_path(self.attn(self.norm1(x)))
        if not self.moe:
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.mlp_drop(self.mlp(self.norm2(x), gate_inp)))
        return x
...
class VisionTransformerMoE(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 gate_return_decoupled_activation=False, gate_return_gated_activation=False,
                 moe_gate_type="noisy", vmoe_noisy_std=1, moe_same_for_all=False):
        """
        Args:
        ...
        self.gate_return_gated_activation = gate_return_gated_activation
        self.moe_same_for_all = moe_same_for_all
        ...
        for i in range(depth):
            if i % 2 == 0:
                ...
                if (not moe_same_for_all) or i == 1:
                    block = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                              ...
                              gate_return_decoupled_activation=self.gate_return_decoupled_activation, gate_return_gated_activation=self.gate_return_gated_activation,
                              moe_gate_type=moe_gate_type, vmoe_noisy_std=vmoe_noisy_std)
                    if moe_same_for_all:
                        block_first = block
                else:
                    assert not self.gate_return_gated_activation
                    block = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                          drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                          moe=True, moe_mlp_ratio=moe_mlp_ratio, moe_experts=moe_experts, moe_top_k=moe_top_k,
                          moe_gate_dim=gate_dim, world_size=world_size, gate_return_decoupled_activation=self.gate_return_decoupled_activation,
                          moe_gate_type=moe_gate_type, vmoe_noisy_std=vmoe_noisy_std, moe_mlp=block_first.mlp)
                blocks.append(block)
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        dim = x.shape[-1]
        # print("self.patch_embed.patch_size is {}".format(self.patch_embed.patch_size))
    ...
    def forward_features(self, x, gate_inp):
        x_origin = x.clone()
        B, nc, w, h = x.shape
        ...
        for block in self.blocks:
            ...
            max_logits = torch.abs(x).max()
            torch.distributed.all_reduce(max_logits)
            if torch.isnan(max_logits):
                torch.save(x_origin, "x_origin.pth")
                save_state_dict = self.state_dict()
                save_checkpoint({
                    'epoch': 111,
                    'state_dict': save_state_dict,
                }, is_best=False, filename='nan.pth.tar',
                save_dir=".", moe_save=True)
                torch.distributed.barrier()
                raise ValueError("logit is nan, stopping training")
    ...
    def forward(self, x, gate_inp=None):
        x = self.forward_features(x, gate_inp)
