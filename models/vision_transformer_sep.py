...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sep=False):
        super().__init__()
        self.sep = sep
        self.sep_path = 0
    ...
    def forward(self, x):
        ...
        if self.sep:
            if self.sep_path == 0:
                # print("run self.sep_path of 0")
                x = x + self.drop_path(self.mlp(self.norm2(x)))
            elif self.sep_path == 1:
                # print("run self.sep_path of 1")
                x = x + self.drop_path(self.mlp2(self.norm2(x)))
            else:
                raise ValueError("self.sep_path if {} is not valid".format(self.sep_path))
        else:
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x


class VisionTransformerSep(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', sep_layers=[1,3,5,7,9,11]):
        """
        Args:
        ...
        for i in range(depth):
            if i in sep_layers:
                blocks.append(Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                              drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, sep=True))
            else:
                blocks.append(Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def set_block_path(self, path=0):
        for module in self.modules():
            if isinstance(module, Block):
                if module.sep:
                    module.sep_path = path
