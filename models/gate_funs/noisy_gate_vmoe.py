...
class NoisyGate_VMoE(BaseGate):
    def __init__(self, d_model, num_expert, world_size, top_k=2, noise_std=1, no_noise=False,
                 return_decoupled_activation=False, return_gated_activation=False):
        ...
        self.return_gated_activation = return_gated_activation
    ...
    def _prob_in_top_k(
        ...
        try:
            ...
            torch.save(clean_values, "clean_values.pth")
            torch.save(threshold_if_in, "threshold_if_in.pth")
            torch.save(threshold_if_out, "threshold_if_out.pth")
            torch.save(noise_stddev, "noise_stddev.pth")

            raise ValueError("wrong cdf")
    ...
    def forward(self, inp):
        ...
        generated_noise = torch.randn_like(clean_logits)
        noisy_logits = clean_logits + (generated_noise * noise_stddev)
        ...
        if self.return_gated_activation:
            assert not self.return_decoupled_activation
            self.activation = gates.reshape(other_dim + [-1, ]).contiguous()
        else:
